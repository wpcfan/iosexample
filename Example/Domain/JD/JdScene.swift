...
class ScriptEvent: Mappable, Codable, Equatable {
    ...
    static func getManualEvent() -> ScriptEvent {
        let ev = ScriptEvent(JSON: [
            "guid": "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF",
            "type": "signal",
            "member": "scenarioActivated",
            "interval": 0,
            "condition": ["name": "id", "type": "s", "operator": "=="],
            "obj": ["name": "scenarioActivated", "type": "s"]
            ])
        return ev!
    }
    required init?(map: Map) { }
